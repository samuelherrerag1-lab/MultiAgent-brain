...
let initPromise: Promise<{ db: DbInstance; type: DbType }> | null = null;
 ...
 *
 * FIX QA: usa Promise-based singleton para evitar race condition entre múltiples
 * llamadas concurrentes a getDb() que podrían crear múltiples instancias.
 */
...
async function createTables(pglite: PGlite): Promise<void> {
  await pglite.exec(TABLE_CREATION);
}
